# Ascension Mount Manager

***

This add on provides smart mount summoning functionality with dynamic environmental awareness (flying vs aquatic vs ground) and an integrated UI button in the Mount Journal.

I will take suggestions for this add on as with any other add that I have made and will made.

***

## **Features**

If you wish to summon a mount via macro, type in chat: **`/ascensionmount`**.

***

### **Ascension Mount Manager:** is a smart mount selector that automatically detects your environment in **World of Warcraft**.

### **AMM** Key Features

* It automatically detects if you are in a flyable area, underwater, or restricted to ground, and picks a suitable mount from your favorites.
* Access its own contextual menu by `Right Clicking` the main frame button in your Mount Journal, which gives access to service mounts (Repair Mammoths/Yaks, Auction House Brutosaurs).
* You can drag the button directly to your action bars to bind the smart summoner.

***

Este add on proporciona una funcionalidad inteligente de invocación de monturas con conciencia ambiental dinámica (volar vs acuático vs tierra) y un botón de interfaz de usuario integrado en el Diario de Monturas.

Aceptaré sugerencias para este add on, al igual que con cualquier otro add on que haya creado o vaya a crear.

***

## **Características**

Si deseas invocar una montura mediante un macro, escribe en el chat: **`/ascensionmount`**.

***

### **Ascension Mount Manager:** es un selector de monturas inteligente que detecta automáticamente tu entorno en **World of Warcraft**.

#### Características principales de **AMM**

* Detecta automáticamente si estás en un área de vuelo, bajo el agua o restringido a tierra, y elige una montura adecuada de tus favoritas.
* Accede a su propio menú contextual haciendo **clic derecho** en el botón del marco principal en tu Diario de Monturas, lo que te da acceso a monturas de servicio (Mamuts/Yaks de reparación, Brutosaurios de Casa de Subastas).
* Puedes arrastrar el botón directamente a tus barras de acción para enlazar el invocador inteligente.

***
