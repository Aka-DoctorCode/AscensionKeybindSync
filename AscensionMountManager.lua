...
-- All flying mounts support Skyriding from patch 11.0, no reason to separate them.
...
-- Aquatic types: all water/swim/waterwalk variants
...
-- Service mount name patterns (lowercase, partial match)
...
addonTable.ascensionMountManager = {
    favoriteMountsCache = {},
    recentSummonHistory = {},
    maxHistorySize = 3
}
...
-- Builds the appropriate mount pool based on current zone environment.
-- Priority: flying > aquatic > ground > all favorites as fallback
function addonTable.ascensionMountManager:buildPriorityPool()
    local flyingPool  = {}
    local aquaticPool = {}
    local groundPool  = {}
...
function addonTable.ascensionMountManager:summonMount()
    ...
    -- If all attempts failed (all mounts recently summoned), pick random
...
function addonTable.ascensionMountManager:createAscensionMacro()
    -- Must use an explicit non-question-mark icon — WoW blocks dragging macros with the auto (?) icon
...
function addonTable.ascensionMountManager:ensureMacroExists()
...
-- Shows a right-click dropdown for service mounts.
function addonTable.ascensionMountManager:showServiceMenu(anchor)
...
function addonTable.ascensionMountManager:integrateUi()
    ...
    -- #1A1A1A dark background gray
    uiButton:SetBackdropColor(0.10, 0.10, 0.10, 1)
    -- #D4AF37 standard golden highlight
    ...
    uiButton:SetScript("OnDragStart", function(self)
        if GetMacroIndexByName("AscensionMount") == 0 then
            addonTable.ascensionMountManager:ensureMacroExists()
        end
        PickupMacro("AscensionMount")
    end)

    -- Left-click: random smart favorite | Right-click: service mounts dropdown
    uiButton:RegisterForClicks("LeftButtonUp", "RightButtonUp")
    uiButton:SetScript("OnClick", function(self, button)
        if button == "RightButton" then
            addonTable.ascensionMountManager:showServiceMenu(self)
        else
            addonTable.ascensionMountManager:summonMount()
        end
    end)
...
local function onEvent(self, event, ...)
    ...
    if event == "ADDON_LOADED" and arg1 == "Blizzard_Collections" then
        addonTable.ascensionMountManager:integrateUi()
    elseif event == "PLAYER_LOGIN" then
        if C_AddOns.IsAddOnLoaded("Blizzard_Collections") then
            addonTable.ascensionMountManager:integrateUi()
        end
        addonTable.ascensionMountManager:buildFavoritesCache()
        C_Timer.After(2, function() addonTable.ascensionMountManager:ensureMacroExists() end)
